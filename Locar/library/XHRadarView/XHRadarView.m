...
#define RADAR_ROTATE_SPEED 120.0f
...
- (void)setup
{
    ...
    {
        self.radius = (self.bounds.size.width - br) / 2.0;
    }
    else
    {
        self.radius = (self.bounds.size.height - br) / 2.0;
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i < sectionsNum ; i++)
    {
        ...
        CGContextAddArc(context, self.bounds.size.width / 2.0, self.bounds.size.height / 2.0, sectionRadius - 5*(sectionsNum - i - 1), 0, 2*M_PI, 0); //添加一个圆
...
- (void)didSelectItemRadarPointView:(XHRadarPointView *)radarPointView {
    NSLog(@"select point %ld", (long)radarPointView.tag);
